...
function createTodo() {
  let inputMsg = prompt("タスクを入力してください", "");
  if (inputMsg) {
    todos.push(inputMsg);
    alert("新しいタスクを追加しました");
    showTodos();
...
function deleteTodo() {
  let deleteMsg = prompt("削除するタスクの番号を指定してください");
  let valueConversion = parseInt(deleteMsg);
  console.log(valueConversion);
  if (
    isNaN(valueConversion) ||
    todos.length <= valueConversion ||
    valueConversion < 0
  ) {
    alert("不正な値のためスキップします");
  } else {
    let deleteTodos = todos.splice(valueConversion, 1);
    alert(deleteTodos + "を削除しました");
    showTodos();
  }
}
